...
-- Type your code below:
with by_genre AS(
SELECT movie_id , count(genre) as total_movies
from genre
group by movie_id
...
AS(SELECT genre , COUNT(movie_id) , RANK() 
OVER(order by COUNT(movie_id) DESC) genre_rank
FROM genre 
GROUP BY genre)
SELECT * FROM count_genre_rank WHERE genre = 'Thriller';
...
select m.title, r.avg_rating, rank() over(order by r.avg_rating desc) movie_rank
from movie m join ratings r on m.id = r.movie_id order by r.avg_rating desc limit 10;
...
as movie_count from movie join ratings on
...
+------------------+-------------------+---------------------+*/
-- Type your code below:
select production_company , count(id) as 
movie_count , rank() over(order by count(id) desc)
prod_company_rank from movie where
...
+---------------+-------------------+ */
-- Type your code below:
select g.genre, count(m.id) as movie_count from movie m join genre g on g.movie_id = m.id join 
ratings r on r.movie_id = m.id where year = '2017' and month(date_published) = '3' 
and country = 'USA' and r.total_votes >1000 group by g.genre order by movie_count desc;
...
+---------------+-------------------+---------------------+*/
-- Type your code below:
select m.title, r.avg_rating, g.genre from movie m join genre g on g.movie_id = m.id
join ratings r on r.movie_id = m.id where title like 'The%'
and avg_rating > 8
 group by m.title
...
-- Type your code below:
select count(id) as movie_count from movie join ratings on ratings.movie_id= movie_id where date_published between '2018-04-01' and 
'2019-04-01' and median_rating=8;
...
from movie m join role_mapping rm
on m.id = rm.movie_id join names n on
rm.name_id = n.id join ratings r on m.id = r.movie_id
...
--------------------------------------------------------------------------------------------*/
-- Type your code below:
select genre, avg_rating, case  when avg_rating < 5 then 'Flop movies'
when avg_rating between 5 and 7
then 'One-time-watch movies'when avg_rating between 7 and 8 then 'Hit movies'
when avg_rating > 8 then 'Superhit movies' end as 'rating_category' 
from genre join ratings on ratings.movie_id = genre.movie_id;
...
OVER(ORDER BY genre ROWS 10 PRECEDING) AS moving_avg_duration
FROM movie AS m 
...
ON g.movie_id = m.id
INNER JOIN ratings AS r ON r.movie_id = m.id WHERE avg_rating > 8
